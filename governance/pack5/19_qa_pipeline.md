...
3. **VNQA** library check: `vnqa/pipeline.py` → `runtime/vnqa_ep_{N}.json` (`:108-119`, timeout 120s). **`VietnameseQAChecker.run_all()` chạy H1-H10** (`vnqa/pipeline.py:372-381`):
   ...
   - ✅ **ĐÃ SỬA (10/7 audit ML #26, per Mr.Long authorization):** nhãn lỗi thời trước đây (`phase_h_version` `"H1-H7 v1.0"` `:402` → `"H1-H10 v1.1"`, orchestrator log `:114`, docstring pipeline `:2`, blueprint `VNQA H1-H9` → `H1-H10`) đã cập nhật khớp thực tế **code chạy H1-H10** (cosmetic, không đổi logic QA). Nợ này CLOSED.
