...
- **Trạng thái:** MỞ — chờ Mr.Long xác nhận: (a) đây là mạch truyện phụ hợp lệ ngoài mô hình bible/18 (không cần field-hoá vào driver_clue), hay (b) cần bổ sung 1 clue-type mới vào `clue_weight_taxonomy` + tính vào budget.
...
- **Nghi vấn cần Mr.Long xác nhận thêm:** không rõ constraint `pillar_distance`/`family_regret_max_per_10_ep` có được Generator/QA THẬT enforce lúc sinh EP02-10 hay không (nếu batch này sinh TRƯỚC khi `bible/11` các invariant này được lock/wire — cần tra lịch sử ngày sinh vs ngày lock bible/11 — KHÔNG tự suy đoán ở đây).
- **Trạng thái:** MỞ — chờ Mr.Long quyết định: (a) chấp nhận batch EP02-10 là ngoại lệ lịch sử (trước khi variety-gate tồn tại), hay (b) cần rà soát/enforcer riêng cho variety trên nội dung ĐÃ SHIP.
