...
- **AUDIT-VERIFIED + OWNER-SIGNED (16/7, per Mr.Long authorization "1 gật"):** CMD_AUDIT độc lập, tự tay tái chạy tại thời điểm audit — `regret_variety_check.py` PASS 0 issue exit 0; 10/10 tập khớp bảng pillar duyệt (frontmatter + ledger); CLIFFHANGER EP07 bất biến (`git show f7bcb29`); pytest 786/8/0; pipeline gate `READY_FOR_OWNER_FREEZE`. Mr.Long gật đóng 16/7. **ĐÓNG CHÍNH THỨC (audit-verified).**
...
- **CHECK #1 — Ý ĐỒ TRUYỆN ĐÃ GIẢI QUYẾT (16/7, per Mr.Long "2 có chủ ý" + CMD_AUDIT tự kiểm tra lại bằng chính văn):** Mr.Long xác nhận bác tài nói vượt "2 câu chuẩn" ở EP07-12 là **MẠCH LEO THANG CỐ Ý**, không phải lỗi. CMD_AUDIT tự đọc episode xác nhận: `output/ep_12/episode.md:83` narrate tường minh *"Bác tài cất lời — một câu — câu thứ ba không nằm trong hai câu như mọi khi"*; chuỗi leo thang EP07 ("dừng trên người đàn ông lâu hơn cả đêm trước") → EP10:75 ("đêm nay không liếc. Bác nhìn... trực tiếp qua gương") → EP12 (câu thứ 3 có chủ đích) = extra_beat_HOOK/quang_memory_arc đã LOCK ở `bible/21`. **HỆ QUẢ:** check #1 KHÔNG được wire thành gate chặn mù (sẽ flag nhầm mạch cố ý). Nếu wire trong tương lai PHẢI exempt extra_beat_HOOK, VÀ **phụ thuộc DEBT-036** (regex `driver_extra_overuse_flag` hiện bắt nhầm lời hành khách — cắm gate lỗi vào = sai). → Không còn là câu hỏi nội dung; chuyển thành task kỹ thuật **sau khi DEBT-036 chốt**: (i) fix regex DEBT-036, (ii) wire exemption-aware. Check #1 GIỮ MỞ nhưng lý do đổi từ "chờ ý đồ" → "chờ DEBT-036".
...
- **AUDIT-VERIFIED + OWNER-SIGNED (16/7, per Mr.Long authorization "1 gật"):** CMD_AUDIT tự đọc `decision_engine.py` — `_live_dialogue_ratio_ep01()` gọi thẳng `cdp.main()`, `build_packet()` override tại dòng 123 (đọc sống, không hardcode); mutation hardcode 0.0 bị 3 test bắt; EP02-11 `wired_into_build_packet:False` khớp claim "đo riêng". Mr.Long gật đóng 16/7. **ĐÓNG CHÍNH THỨC (audit-verified).**
...
- **⚠ ĐẢO NGƯỢC TOÀN BỘ (17/7, R216 — Mr.Long: "tập sinh trước = gốc, sinh sau bịa khác = LÁO"; Boss chốt Hướng A):** Cách đóng phần #2 ngày 16/7 (reframe EP01 New York→Hà Nội) **LÀ VI PHẠM R216 — SAI HƯỚNG, đã REVERT.** Toàn bộ lập luận cũ trong DEBT này (mục "Vì sao có khả năng" gọi EP01 là "bản nháp"; mục "Field-hoá theo EP11-50 làm nguồn CHÍNH") = **codify NGƯỢC**, KHÔNG còn hiệu lực. Bằng chứng git quyết định hướng: EP01 sinh `3306d91` **26/6** (trước); EP11-50 auto-gen `c6a5d59` **27/6 17:06** (sau 1 ngày); bible/21 memory-arc `4cf9a78`+`148a80c` **27/6 tối** (sau cùng). → **EP01 = GROUND TRUTH.** Boss thêm: "chết ở nước ngoài là bình thường" → New York KHÔNG phải world-error, không có ngoại lệ lật R216.
- **HƯỚNG A ĐÃ CHỐT — reconcile NGƯỢC toàn bộ về EP01 (canon đúng):**
  - **EP01 (3 file) — ĐÃ REVERT về v7 gốc (17/7):** khôi phục "ngồi ghế số bảy" + "Hạ Vy mất taxi New York / du học Hoa Kỳ / đồng hồ 7:10". `bible/00` golden_reference lùi **v8→v7** (`ep_01_v7_final_round3_lock`). Enforcer build-ngược `tools/canon_consistency_check.py` + test (khoá nhầm hướng Hà Nội) **ĐÃ GỠ** (`git rm`).
  - **Enforcer đúng hướng — ĐÃ DỰNG + WIRED (17/7 G1):** `tools/cross_episode_canon_check.py` (ratchet) + `tests/test_cross_episode_canon_check.py` (10 test mutation-proof: allowlist LOAD-BEARING — gỡ allowlist → divergence thật FLIP FAIL; anchor=earliest; evaluate TRACKED/FAIL). **Đã wire vào `tools/ci_gate.py` CHECKS** (chạy pre-push): chặn MỌI divergence MỚI, track divergence đang reconcile (`KNOWN_PENDING={kp_seat}`) dạng [TRACKED-DEBT-035] — không im lặng. Codify hiến pháp `bible/00 rule_R216_earlier_episode_is_ground_truth_hardlock`. **Giới hạn trung thực (R215.1):** gate prose chỉ cover ghế Khải Phong (công thức 1 câu tin cậy); death Hạ Vy KHÔNG gate regex prose (episode đa-hành-khách → false-positive EP27/EP47, đã thử+gỡ) → enforce tầng structured R207 sau reconcile.
  - **CÒN LẠI — RECONCILE PENDING (khối lượng lớn, đang scope bằng agent 17/7):**
    - **Phần #1 (ghế):** reconcile 49 tập EP02-50 + mọi locus bible (bible/03 dòng 92-94, bible/21 M-arc, roster) từ "ghế thứ ba" → **"ghế số bảy"**.
    - **Phần #2 (cái chết):** reconcile bible/03 core_wound (dòng 122-129 Hà Nội/xe máy/phố Huế/ep41) + bible/21 + 49 tập từ Hà Nội/xe máy → **New York/taxi/du học**.
    - **Phần #3 (M13 memory-arc):** XÁC NHẬN xung đột cấu trúc THẬT — bible/21 M13 "Khải Phong CHỞ Hà ngày Hạ Vy mất" (KP tự lái) **bất khả** với EP01 (Hạ Vy chết trong TAXI ở New York, KP không lái). Memory-arc reveal lõi phải rework theo EP01.
  - **Chưa thực thi reconcile bible/21 + regen 49 tập** — theo R_SUPREME R7 cần proposal `.md` grounded bằng scope agent (đang chạy) + Boss duyệt cụ thể trước khi phá bible/21 LOCK. Đợt commit 17/7 CHỈ gồm: revert EP01, gỡ enforcer ngược, dựng enforcer đúng, reframe note bible/03, cập nhật registry — KHÔNG đụng bible/21 + 49 tập.
...
- **Trạng thái:** ~~MỞ~~ → **CLOSED (16/7, xem block CLOSED bên dưới)** — không chặn Bước 4 (extra_beat_HOOK vẫn field-hoá đúng và có test riêng chứng minh cơ chế hoạt động độc lập với lỗi này); đã sửa bằng speaker-tracking (Variant B).
...
- **CLOSED (16/7, per Mr.Long "merge B luôn, tự audit trước khi land" + "land B luôn khi DEBT-035 xong" — agent EDIT build, CMD_AUDIT tự tay audit + land):**
  - **Fix:** `tools/audit_hidden_bugs.py` thay cơ chế detect quote bác tài từ regex `DRIVER_QUOTE_PATTERN` ("nhảy tới dấu `"` gần nhất phía sau cụm kích hoạt", băng qua newline/section) sang **speaker-tracking `driver_quotes()`** (adjacency): mỗi quote gán cho speaker qua attribution GẦN NHẤT phía trước — nhánh (1) inline cùng đoạn (`_inline_driver`, cho phép mô tả xen giữa verb↔quote để cứu EP21, chặn `không nói` + attribution hành khách xen sau) hoặc (2) đầu-đoạn xét ĐOẠN LIỀN TRƯỚC (`_is_driver_para`). **Chữ ký công khai `split_driver_extra_quotes()` / `driver_extra_overuse_flag()` GIỮ NGUYÊN** (R211), chỉ đổi nguồn list quote. `DRIVER_QUOTE_PATTERN` giữ lại (LEGACY) làm mutation anchor.
  - **Đo lại thật (toàn 50 EP, R215.3):** flag check[3] = **`[12, 14, 16, 17, 18, 35]`** (MATCH), **passenger-FP = 0** (regex cũ 50), **5 lời foreshadow bác tài thật được cứu** (EP11/12/21/30/50 — Variant A sót cả 5). B strictly better A: 0 false-negative vs A=5, cùng loại hết 50 FP, flag y hệt.
  - **Test (mutation-proof, R215.1):** `tests/test_audit_hidden_bugs_driver_speaker_tracking.py` (mới) — 11 case BEHAVIORAL (3 FP-bỏ + 5 driver-giữ gồm 2 case A sót + negation guard + mutation anchor). DEBT-032 test cũ `test_audit_hidden_bugs_extra_beat_hook.py` reconcile (bỏ pin vào `_LEGACY_PATTERN` bug → so với công thức tổng quát trên nguồn quote mới, GIỮ intent "nhánh HOOK no-op cho non-hook EP") → PASS 0 regression.
  - **CMD_AUDIT TỰ TAY VERIFY (không tin lời agent — R215.4):** tool 50 EP flag `[12,14,16,17,18,35]`; 22 test liên quan PASS; chữ ký hàm bất biến; **mutation-flip: ép `driver_quotes` về regex legacy → 4 test FAIL** (FP-drop + negation-veto + mutation-anchor bắt được), restore → sạch.
  - **Doc-code parity (R215.2):** cập nhật docstring module + comment/label check[3] ("strict regex" → "speaker-tracking") + docstring `split_driver_extra_quotes()`; đăng ký test mới vào `governance/file_index.yaml` (total → 395 gồm cả canon DEBT-035), registry PASS 0/0/0.
  - **Giới hạn còn ghi (minh bạch):** B giả định lời bác tài luôn INLINE cùng đoạn hoặc đầu-đoạn ngay sau đoạn-dẫn bác tài — nếu 1 EP tương lai đặt lời bác tài cách attribution qua 1 đoạn narration chèn giữa, B sẽ sót (50 EP hiện tại KHÔNG có case này). 6 EP còn flag `[12,14,16,17,18,35]` là câu hỏi RIÊNG (true R55 hay artifact ranh giới HOOK) — độc lập DEBT-036.
...
- **Trạng thái: CLOSED (16/7, per Mr.Long "gọi agent chạy song song, đẩy nhanh tiến độ" — 2 agent build trong worktree, CMD_AUDIT tự tay audit + reconcile + verify trước khi land):**
  - **(a) DONE:** `test_preflight_has_regret_variety_gate_wired` nâng thành BEHAVIORAL — dựng ledger vi phạm cố ý + chạy `svhmp_preflight_qa.py` end-to-end qua subprocess (`runpy`, patch `story_planner.EVENT_LEDGER`), assert exit≠0 + có `R-REGRET-VARIETY`; nhánh sạch assert không chặn. **CMD_AUDIT tự tay verify mutation-flip:** unwire (`_rv_issues=[]`) → test MỚI FAIL (bắt được), test grep CŨ vẫn PASS (chứng minh grep mù) → test mới hơn hẳn. 2 assert grep giữ làm smoke BỔ SUNG *bên trong* test behavioral (không phải standalone grep-only guard, đúng R215.5).
  - **(b) DONE:** `tests/test_no_greponly_wiring_guards.py` (mới) — AST-detector + ratchet allowlist `KNOWN_GREPONLY_GUARDS`, FAIL nếu có test grep-only-wiring MỚI ngoài allowlist. **CMD_AUDIT tự tay verify ratchet:** thêm fake grep-guard → meta-test FAIL; gỡ → PASS. Đăng ký `governance/file_index.yaml` (tier 3, total 391→392), `architecture_registry_check.py` PASS 0/0/0.
  - **INSTANCE THỨ 2 phát hiện qua sweep (16/7):** `tests/test_gate_wired_g2.py::test_character_gate_reachable_via_wrapper` cũng grep-only (assert WIRE_MARKERS + `"svhmp_v13_render.py"` in wrapper source) — CMD_AUDIT đọc xác nhận. Có behavioral coverage ở test KHÁC cùng file (`test_run_character_gate_blocks_on_real_low_completeness_strict`) nên character-gate không mù về hành vi, nhưng hàm `_reachable_via_wrapper` này vẫn nên nâng behavioral. **Đưa vào allowlist meta-test (tracked), CHƯA fix behavioral — nợ con DEBT-037.b MỞ**, không chặn (đã có behavioral coverage nơi khác).
- **HỆ QUẢ DEBT-032 check#2:** điều kiện PASS-với-điều-kiện đã được xử lý (guard giờ behavioral + có meta-checker chống tái diễn) → check#2 **đóng hoàn toàn**.

## DEBT-038: `tools/promotion_guard.py::_git` — `subprocess.run(text=True)` thiếu `encoding=` (cp1252 crash lớp cũ lọt qua sweep 9/7) + 11 instance backlog trong `tools/`

- **Phát hiện:** 16/7, CMD_AUDIT, khi debug "push treo" lúc land DEBT-036. Trace pre-push thật: `UnicodeDecodeError: 'charmap' codec can't decode byte 0x90 ... in _readerthread`.
- **Đính chính chẩn đoán (R215.3, tự tay tái hiện):** "push treo" **KHÔNG phải** do crash này — thủ phạm chính là `pre-push` chạy full CI gate (pytest 817 test ~7.5 phút) mà CMD_AUDIT cắt timeout ở 2-3 phút nên tưởng deadlock. Đã để push chạy đủ → PASS, land f20eb48. Crash cp1252 là **non-fatal**: nó chết trong reader-thread nền → `subprocess.run` trả stdout rỗng/cụt (KHÔNG raise ra main thread), `promotion_guard` vẫn in OK. Nhưng đó là **silent data-loss**: `_range_messages` đọc `git log --format=%B` của range push → nếu commit message có dấu tiếng Việt, guard đọc NHẦM message rỗng → có thể BỎ SÓT vi phạm lock/tag chưa uỷ quyền. `ci_gate.py` đã đúng (utf-8+errors=replace); `promotion_guard._git` là instance LỌT.
- **Process failure:** docs/ENVIRONMENT_GOTCHAS.md (mục cp1252, 9/7) đã ĐỀ XUẤT "grep repo tìm `text=True` không kèm `encoding=`" nhưng chỉ là lời khuyên, KHÔNG có enforcer → lọt `_git` + còn 11 instance khác. Đúng lớp R215 (claim/đề xuất không có gate).
- **Fix (16/7, per Mr.Long "fix hook"):**
  - `_git` thêm `encoding='utf-8', errors='replace'` (parity `ci_gate.py`). Backup R8: `scratchpad/promotion_guard.py.bak`.
  - **Test instance (mutation-proof, R215.1):** `tests/test_promotion_guard_git_utf8.py` — 3 case: roundtrip repo git thật có commit message dấu tiếng Việt / cp1252-không-faithful cross-OS / introspect kwargs runtime. **CMD_AUDIT tự tay mutation-flip:** revert `_git` → 2 test FAIL (roundtrip Windows + kwargs), restore → sạch.
  - **ENFORCER LỚP (biến đề xuất 9/7 thành gate, R215.1):** `tests/test_no_text_true_without_encoding.py` — AST-scan mọi `tools/**/*.py` tìm `.run/.Popen/.check_output/.call` có `text=True`/`universal_newlines=True` thiếu `encoding=`; **ratchet allowlist theo file+count**, FAIL nếu có FILE mới vi phạm HOẶC file allowlist tăng số instance. Mutation-flip verify: thả file probe vi phạm vào `tools/` → FAIL; gỡ → 3 PASS.
- **Backlog còn MỞ (11 instance legacy trong allowlist `KNOWN_TEXT_TRUE_NO_ENCODING`, sửa dần):** `auditor.py`(2), `dashboard/server.py`(2), `event_logger.py`(1), `music_loop.py`(2), `pre_render_audit.py`(1), `project_bootstrap.py`(1), `sfx_acquire.py`(2). Enforcer chặn thêm mới; mỗi lần sửa 1 instance thì hạ số trong allowlist (`test_known_backlog_not_stale` nhắc nếu quên). Chưa chặn push (các tool này không chạy trên hook, trừ khi wire sau) → ưu tiên thấp.
- **Trạng thái:** instance `_git` **CLOSED** (fixed + mutation-proof test + enforcer lớp); backlog 11 instance **MỞ** (tracked qua allowlist, không chặn). file_index total 395→397; registry 0/0/0.

## DEBT-039: 2 gate trong ci_gate.CHECKS chưa có mutation-proof test riêng (`registry`, `R205_char`) — tracked qua META-GATE allowlist

- **Phát hiện:** 17/7, khi dựng META-GATE (đề xuất #2, `governance/AUDIT_ROOT_CAUSE_SYNTHESIS_17_07.md` mục 3): map mọi stage `ci_gate.CHECKS` → 1 test mutation-proof đã đăng ký. Khi soi từng gate, 2 stage KHÔNG có test nào dựng vi phạm rồi assert gate bắt.
- **Bằng chứng (đã đọc test thật, R215.4):**
  - `registry` (`tools/architecture_registry_check.py`): chỉ có test POSITIVE — `tests/test_ci_suite.py` chạy nó qua subprocess assert exit 0 trên repo sạch. KHÔNG có test dựng MISSING/UNMAPPED/DUP giả lập rồi assert exit 1.
  - `R205_char` (`tests/test_character_manager_r205.py`): chủ yếu invariant/property POSITIVE (đếm 139 passenger, naming=0 lỗi, enrich, schema fields); DUY NHẤT 1 negative (`enrich field sai -> KeyError`). Không phải confusion-battery dựng vi phạm rồi assert detector bắt (khác R203/206/207/208).
- **Không chặn / vì sao chưa fix ngay:** 2 gate này vẫn chạy thật trong ci_gate (registry đang enforce 0/0/0 strict; R205 khẳng định các invariant roster). Chỉ là THIẾU 1 test mutation-proof chứng minh chúng bắt được vi phạm — nợ chất lượng test, không phải lỗi hành vi. Được TRACK tường minh trong `governance/gate_mutation_map.yaml::known_no_mutation_test` (kèm ref DEBT-039). META-GATE `tools/gate_mutation_registry_check.py` + `tests/test_every_gate_has_mutation_test.py` FAIL nếu gỡ entry allowlist mà chưa có test (allowlist load-bearing, đã mutation-flip verify) → không thể quên.
- **Đề xuất:** task riêng — (registry) dựng fixture registry vi phạm (thêm entry MISSING / file UNMAPPED) → assert exit 1; (R205) nâng thành confusion-battery hoặc thêm negative cases dựng profile vi phạm (thiếu Tier1, đổi field khoá) → assert detector bắt. Mỗi lần fix xong: chuyển stage từ `known_no_mutation_test` sang `mapped` với `test_node` collectable.
- **Trạng thái:** MỞ (2 gate tracked allowlist, không chặn push; META-GATE ngăn thêm gate mới không mutation-test). file_index total 397→400; registry 0/0/0.

## DEBT-040: DNA rules (ALWAYS_5 / NEVER_7 / GHOST_RULES_3 / ENDING_RULES / SERIES_RULES) = LLM_JUDGE_ONLY — 0 machine gate, enforcement chỉ trong `prompts/qa.md`

- **Phát hiện:** 17/7, khi dựng CLAIM-ENFORCER registry (đề xuất #1+#4, `governance/AUDIT_ROOT_CAUSE_SYNTHESIS_17_07.md` mục 3; `governance/claim_enforcer_map.yaml`). 5 claim DNA trong `bible/00_constitution.yaml` ghi ngôn từ khẳng định ("QA HARD-FAIL", "vi phạm = HARD-FAIL", immutable) nhưng KHÔNG có tool/gate máy nào hậu-kiểm nội dung — enforcement thực tế chỉ nằm trong prompt LLM QA Lock.
- **Bằng chứng (đã tự đọc code + verify, R215.4):**
  - `bible/00 enforcement.qa_lock.HARD_FAIL_triggers` liệt kê "any NEVER violation / any GHOST_RULES violation / missing ALWAYS principle / aftertaste resolved (ENDING_RULES) / bell>1 / ghost_visual>1 / driver câu thứ 3" — nhưng qa_lock = `prompts/qa.md` (LLM judge). Đã xác nhận `prompts/qa.md` TỒN TẠI + chứa ALWAYS/NEVER/GHOST/PHASE 12 (grep). 0 checker máy quét content tìm gore/jump_scare/exorcism...
  - `ENDING_RULES.unresolved_memory` ghi "QA HARD-FAIL nếu beat_5 close hoàn toàn" — 0 tool (synthesis G6).
  - **NGUY HIỂM MASK-AS-ACTIVE (SERIES_RULES):** `tools/post_render_gate.py` TRÔNG như gate SERIES_RULES nhưng đọc code thấy KHÔNG enforce: L85 driver "2 câu" hardcode `results.append((True, ...))` LUÔN PASS (synthesis G3); L91 bell đếm `>=1` (cận DƯỚI) KHÔNG kiểm `max_per_episode==1`; L101 ghost_visual đếm `>=1` (cận DƯỚI) KHÔNG kiểm `max==1` (synthesis G4). Ràng buộc MAX/exact của SERIES_RULES chỉ còn qua qa_lock LLM.
