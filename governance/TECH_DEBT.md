...
- **HƯỚNG A ĐÃ CHỐT — reconcile NGƯỢC toàn bộ về EP01 (canon đúng):**
  ...
  - **Enforcer đúng hướng — ĐÃ DỰNG:** `tools/cross_episode_canon_check.py` + `tests/test_cross_episode_canon_check.py` (5 test, anchor = tập sớm nhất KHÔNG phải số đông). CHƯA wire ci_gate vì còn phân kỳ THẬT chưa reconcile hết.
