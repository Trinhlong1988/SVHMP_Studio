...
- **Trạng thái:** **MỘT PHẦN CLOSED (16/7, per Mr.Long "sai thì sửa tuân đúng từ đâu" + "GO land + bless v8", CMD_AUDIT audit + land).**
  - **Phần #2 (cái chết Hạ Vy) — CLOSED:** EP01 3 file (`episode.md`/`episode_golden_text.md`/`episode_tts_ready.md`) đã reframe khung "du học Hoa Kỳ / New York / Kennedy / taxi / cao tốc / sân bay" → "đêm mưa cuối tháng tư ở Hà Nội, cô ấy đi xe máy về nhà, tới ngã tư bị xe tải đâm ngang, mất ngay tại nơi đó" — khớp canon bible/21 (Hà Nội / xe máy / 12-4). GIỮ reveal budget EP73 (KP tách khỏi hiện trường, mẹ Hạ Vy báo tin — KHÔNG lộ KP lái / cả hai cùng chết). Golden bless **v7→v8** (`v8_canon_reconcile_lock`, bible/00 golden_reference cập nhật). **Enforcer khóa lỗi:** `tools/canon_consistency_check.py` (proximity Hạ-Vy + chết + token nước-ngoài, chống false-positive hành khách khác) + `tests/test_canon_consistency_check.py` 8 test mutation-proof. **CMD_AUDIT tự tay verify:** EP01 gốc → enforcer FLAG 2 (New York+Kennedy); EP01 đã sửa → 0; 50 EP PASS 0; R86 broad 0; R41 2899/2899; dialogue_ratio Δ+0.0017; full suite 797 passed/0 failed.
  - **Phần #1 (ghế 7 vs 3) — VẪN MỞ:** EP01 dòng ~100 vẫn "ghế số bảy" trong khi 49/50 tập dùng "ghế thứ ba" — KHÔNG nằm trong proposal cái-chết đã duyệt, chưa sửa (chờ Mr.Long quyết fix follow-up EP01 hay không).
  - **Phần #3 (bible/21 M13 "Khải Phong chở Hà" vs EP31-41 "Hạ Vy một mình") — VẪN MỞ:** rủi ro plan-vs-written, chờ khi viết EP61-75 (chưa actionable).
