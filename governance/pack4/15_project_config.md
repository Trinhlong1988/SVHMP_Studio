...
**Workflow:** khai `project_config.yaml` → `validate_project_config.py` (đủ field?) → engine đọc config → render theo project.
**Mandatory Rules:** 5 field bắt buộc (bảng dưới) phải hiện diện; thiếu 1 field = INVALID (exit 1). CẤM engine hard-code các giá trị này.
**PASS Criteria:** mọi `project_config.yaml` hiện diện đủ 5 field → validator exit 0.
**FAIL Criteria:** thiếu/sai kiểu 1 field → validator exit 1, BLOCK dùng config đó.
**Examples:** genre=horror + dialect=south → SVHMP; đổi genre=romance + dialect=north → cùng engine, khác output.
