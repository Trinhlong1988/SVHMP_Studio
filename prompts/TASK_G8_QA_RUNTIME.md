...
**QUYẾT ĐỊNH Mr.Long (9/7, qua CMD_AUDIT — bằng chứng: `bible/00_constitution.yaml` dòng 1671, R96
v3.3 "nới `noisy_pause_max` từ 0 lên 1", evidence commit `8c2e929f`):** dùng **`noisy<=1`** làm chuẩn
hợp nhất (khớp đúng bible hiện hành). `qa_pause_silence.py` (đang `noisy==0`) mới là bản LỖI THỜI cần
...
### D6 — Xử lý lớp batch legacy (`deep_200_rounds.py`, `deep_200_rounds_all_rules.py`, `verify_50_rounds.py`, `sequential_full_auto.py` — 4 file, không phải 3, xem quyết định dưới)
...
**QUYẾT ĐỊNH Mr.Long (9/7, qua CMD_AUDIT):** **KHAI TỬ cả 4 file.** Bằng chứng đã tự `grep` toàn
repo trước khi trình: đúng **4 file** đăng ký `governance_audit`/`generation` manifest (không phải
3 như liệt kê ban đầu — bổ sung `tools/deep_200_rounds_all_rules.py`), **0 live caller thật** cho cả
4 (chỉ xuất hiện trong doc/manifest, không có file `.py` nào import/gọi). `deep_200_rounds.py` đã
stale-mark từ trước (`bdc36e8`). CMD_BUILD thực thi: xóa 4 file khỏi `governance/manifests/
governance_audit_manifest.yaml`/`generation_manifest.yaml`, chuyển ghi nhận sang
`governance/deprecation_report.md`, xóa file `.py` + entry `file_index.yaml` liên quan. Chạy lại
`architecture_registry_check.py` xác nhận 0/0/0 sau khi xóa (không để MISSING xuất hiện do registry
còn trỏ file đã xóa).
