...
## CASE ĐÃ TÌM THẤY — "G8 chèn disclaimer" (DEBT-023, CMD_BUILD_2 11/7, per Mr.Long quyết hướng b)
**Enforcer KHÔNG hề tồn tại (không phải "hẹp hơn" — mà "chưa xây").** `bp9/content_policy.yaml:71-76`
`disclaimer_rule` khẳng định mọi tập tier medium/high PHẢI có disclaimer hư cấu, và ghi "vị trí + câu
chữ do **G8 (QA Runtime wire) quyết định**". Rà thật (Mr.Long tự grep xác nhận + CMD_BUILD_2 grep lại):
`tools/vnqa/*`, `tools/qa_*.py` = **0 kết quả xử lý "disclaimer"**; `tools/publish_gate.py` (nơi
được cho là chèn) **KHÔNG tồn tại trên đĩa** (vẫn `planned` trong blueprint). → "G8 sẽ chèn lúc phát
hành" là **lời hứa chưa ai xây** — đúng lớp R197/R210 (claim có, enforcer không). Hậu quả thật: 25/50
tập đã render thiếu disclaimer → rủi ro pháp lý.
**Đã xử lý (hướng b, không chờ cơ chế chưa tồn tại):** thêm disclaimer hư cấu (câu chữ khác nhau từng
tập, high-tier thêm ý tôn kính) vào 25 tập medium/high + enforcer `tests/test_disclaimer_present_25ep.py`
(mutation-proof, khóa 25 tập luôn có disclaimer). Còn NỢ: cơ chế tự động chèn disclaimer cho tập MỚI
(ep 51-90 tương lai) + tự phân loại tier — CHƯA xây (Mr.Long quyết sau, ngoài phạm vi DEBT-023).
