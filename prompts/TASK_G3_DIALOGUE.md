...
> có **11 entry** trong `CHECKS` — `registry, blueprint, R199_tail, R203_conf, R205_char, R206_voice,
...
| Gate 1 cửa | — | ❌ `ci_gate.py` hiện có **11 entry** trong `CHECKS` (`registry, blueprint, R199_tail, R203_conf, R205_char, R206_voice, R207_canon, R208_age, project_config, G2_roster, g5_supernatural, G4_world`) — KHÔNG có entry `dialogue`/`G3_dialogue` (sửa theo skeptic-1: một bản nháp trước liệt thiếu 7/11 entry) |
...
- Chạy và lưu: `python tools/ci_gate.py` (xác nhận đủ **11 stage** hiện có trong `CHECKS` —
...
'tools/g3_dialogue_check.py')` (đặt sau `R208_age`, không xoá/đổi thứ tự 11 entry hiện có). **R191
...
`reports/G3_DIALOGUE_REPORT.md`: tổng hợp số liệu thật D0 (before/after, đủ 11 stage ci_gate) +
...
D0 baseline log lưu (before/after, đủ 11 stage ci_gate thật) + số MATCH/MISMATCH thật ✅ · D1 SSOT
...
(11 entry CHECKS thật), `tools/architecture_registry_check.py`,
