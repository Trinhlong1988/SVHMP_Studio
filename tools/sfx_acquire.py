...
def generate_via_ace_step(asset_id, spec):
    ...
    # NOT IMPLEMENTED — tracked at governance/TECH_DEBT.md#DEBT-003 (không tự
    ...
    logging.warning(
        f"[NOT-IMPLEMENTED][DEBT-003] {asset_id}: gradio_client call to ACE-Step "
        f"API chưa implement — xem governance/TECH_DEBT.md#DEBT-003"
    )
    return None
...
def acquire_asset(asset_id, dry_run=False, force=False):
    ...
    if spec.get("ai_generate"):
        ...
        # NOT IMPLEMENTED — tracked at governance/TECH_DEBT.md#DEBT-003 (compositing
        # từ raw assets có sẵn — cần raw assets thật ready trước, ngoài phạm vi 1 fix
        # code). em sẽ implement sau khi raw assets ready.
        logging.warning(
            f"[NOT-IMPLEMENTED][DEBT-003] {asset_id}: build_from compositing chưa "
            f"implement — xem governance/TECH_DEBT.md#DEBT-003"
        )
        return False
    else:
