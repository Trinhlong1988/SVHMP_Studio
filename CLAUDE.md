...
> 5. **Guard "đã-wired" bằng text-grep KHÔNG tính là mutation-proof — phải BEHAVIORAL (rút từ audit DEBT-032 check#2, 16/7, CMD_AUDIT tự tay tái hiện).** Một test chỉ `assert "<tên_hàm>" in source` / `assert "<token>" in source` để chứng minh enforcer đã được nối vào pipeline là MÙ với việc unwire: thay lời gọi thật bằng no-op (vd `_rv_issues = check_regret_variety(...)` → `_rv_issues = []`) mà GIỮ dòng `import` + dòng `issues.append(...)` → token vẫn còn trong source → test vẫn PASS trong khi gate đã bị unwire hoàn toàn. Đã tự tay chứng: sau khi gỡ call thật, cả 2 assert `"check_regret_variety" in src` + `"R-REGRET-VARIETY" in src` (test `test_preflight_has_regret_variety_gate_wired`) đều True → guard MÙ. Đây đúng lớp "run_all rỗng" điểm 1 cảnh báo, và trớ trêu là chính test tự xưng "unwire-guard, mirror test_g5_gate_has_..._wired". **BẮT BUỘC:** mọi test tự nhận "unwire-guard"/"wiring test"/"composition guard" PHẢI dựng input vi phạm thật rồi chạy ĐÚNG entrypoint composition end-to-end (vd chạy `svhmp_preflight_qa.py` cả pipeline trên ledger có vi phạm cố ý) + assert nó THỰC SỰ FAIL (exit≠0 / issue xuất hiện) khi wired VÀ không fail khi input sạch; static `in source`/regex chỉ được dùng BỔ SUNG, KHÔNG thay thế. **ĐÃ CÓ ENFORCER meta (16/7):** `tests/test_no_greponly_wiring_guards.py` = AST-detector + ratchet allowlist, FAIL nếu có test grep-only-wiring MỚI (ngoài `KNOWN_GREPONLY_GUARDS`) — mutation-proof đã verify (thêm fake grep-guard → FAIL; gỡ → PASS). Guard gốc `test_preflight_has_regret_variety_gate_wired` đã nâng behavioral. Backlog grep-only cũ nằm trong allowlist kèm ref DEBT-037 (vd `test_character_gate_reachable_via_wrapper` — instance thứ 2, chờ nâng behavioral).
> 6. **"Đề xuất quy trình" / "nên quét định kỳ" / remediation-suggestion viết trong docs/gotchas/proposal mà KHÔNG kèm gate = CÙNG LỚP rule-without-enforcer (rút từ DEBT-038, 16/7, CMD_AUDIT tự tay).** Bằng chứng: mục cp1252 trong `docs/ENVIRONMENT_GOTCHAS.md` (9/7) đã ghi "grep repo tìm `text=True` không kèm `encoding=` — nên quét định kỳ" nhưng chỉ là LỜI KHUYÊN, không có test/gate nào chạy nó → suốt hơn 1 tuần lọt `tools/promotion_guard.py::_git` (đúng hàm chạy trên pre-push hook, đọc commit message tiếng Việt → cp1252 silent data-loss) + 11 instance khác trong `tools/`. Một "đề xuất quét" nằm im trong prose ĐỌC NHƯ đã được bảo vệ nhưng thực tế không ai chạy — y hệt claim "đã enforce" không có gate ở điểm 1. **BẮT BUỘC:** mọi câu dạng "nên quét/nên kiểm/cần rà định kỳ/đề xuất quy trình chống tái diễn" khi viết ra PHẢI ngay lập tức 1 trong 2: (a) biến thành enforcer máy (test/gate ratchet + mutation-proof — vd `tests/test_no_text_true_without_encoding.py` AST-scan + allowlist file+count, đã verify probe→FAIL), hoặc (b) gắn nhãn tường minh **"CHƯA CÓ ENFORCER — mới là đề xuất"** + mở DEBT tracked ngay tại chỗ. CẤM để "đề xuất quy trình" trôi như bằng chứng đã-phòng-thủ. Hệ quả kèm: khi debug thao tác chạy-lâu (git push/pre-push = full CI gate ~8 phút), CẤM tuyên bố "treo/deadlock/thiếu credential" trước khi biết thời lượng kỳ vọng + có trace thật (R9 evidence-first, R10 uncertainty→STOP) — đã 2 lần vội kết luận sai trong phiên 16/7.
...
> **R216 EARLIER-EPISODE = GROUND TRUTH — LÀM RÕ R65 + R207 (KHÔNG phải luật mới; Mr.Long 17/7, rút từ vụ suýt sửa ghế/nơi-chết EP01 theo EP02-50):** Nguyên tắc "cấm bịa, nhất quán xuyên tập" ĐÃ CÓ SẴN — **R65** (`bible/00` dòng 724, `R65_total_consistency_no_invention_hardlock`: mọi chi tiết PHẢI nhất quán tổng thể, CẤM bịa) + **R207** (`story_consistency_validator.py`, `LOCKED_FIELDS` gồm `death_date/death_type/hometown/pain_core`). R216 KHÔNG đẻ thêm luật — chỉ **làm rõ 1 điểm chưa quy định + vá 2 lỗ hổng enforcer đã-có** đã để lỗi lọt: **Tập sinh TRƯỚC luôn là GỐC/nguồn sự thật cho tập sinh SAU. Tập sau BỊA/mâu thuẫn logic với tập trước = LÁO — bug nằm ở TẬP SAU (hoặc ở bible/tool đã codify NGƯỢC), KHÔNG phải ở tập trước.**
> **VÌ SAO LỌT DÙ ĐÃ CÓ RULE + ENFORCER (kiểm chứng bằng đọc code 17/7, KHÔNG suy đoán):** (a) `story_consistency_validator` so `current` với `baseline = profile trong bible/registry` → mặc định coi BIBLE là chuẩn, KHÔNG có khái niệm "tập sớm nhất thắng"; khi chính bible bị codify ngược (ghi Hà Nội) thì enforcer *xác nhận* hướng sai, coi EP01/New York là bên lệch. **Điểm DUY NHẤT thật sự thiếu = tie-break direction.** (b) Chi tiết "ghế số bảy" vs "ghế thứ ba" là **PROSE**, không phải structured `LOCKED_FIELD` → validator (chỉ đọc dict nhân vật có cấu trúc) chưa bao giờ nhìn thấy nó; death-location EP01 cũng nằm trong prose. **Enforcer có tồn tại nhưng không đọc prose tập = coverage gap.** R216 = (1) làm rõ tie-break (anchor = tập sớm nhất, KHÔNG phải bible/số đông) + (2) `cross_episode_canon_check.py` mở rộng R65/R207 sang prose.
> **QUY TẮC CẤM RÕ RÀNG (Mr.Long 17/7):** **VẠN VẬT — NGƯỜI — SỰ KIỆN** (mọi thực thể: nhân vật + tên + tuổi + quê + nghề + giọng + xưng hô + quan hệ; mọi đồ vật + đặc điểm; mọi địa điểm; mọi sự kiện + mốc thời gian + cách/nơi xảy ra; mọi con số/chi tiết) **một khi đã SINH RA / được thiết lập ở tập TRƯỚC thì đó là LOGIC bất biến. CẤM TUYỆT ĐỐI sinh ở tập sau một phiên bản SAI / LÁO / BỊA khác với tập trước.** Tập sau chỉ được (a) giữ y nguyên, hoặc (b) HÉ LỘ THÊM (reveal) không mâu thuẫn — TUYỆT ĐỐI KHÔNG đổi khác/tô lại/bịa mới. Vi phạm = dữ liệu láo, phải sửa TẬP SAU về khớp tập trước. Hệ quả BẮT BUỘC:
> 1. **CẤM sửa tập trước để khớp tập sau.** Khi phát hiện tập sau khác tập trước về 1 canon-fact (chỗ ngồi, tên, quan hệ, mốc thời gian, cách/nơi chết, vật định danh), mặc định **tập trước ĐÚNG, tập sau SAI** — đối chiếu + sửa TẬP SAU về khớp tập trước, KHÔNG đụng tập trước. Ngoại lệ DUY NHẤT: tập trước chứa lỗi sự-thật-thế-giới đã kiểm chứng độc lập (vd bối cảnh) — và phải nêu bằng chứng thế-giới-canon, KHÔNG phải "vì số đông tập sau viết khác".
> 2. **CẤM codify bible/tool theo "số đông tập" hay tập sau.** Lỗi gốc đã xảy ra: `bible/03_character_bible.yaml` (dòng 140-142) tự thú field-hoá `core_wound` "theo phiên bản **EP11-50**... KHÔNG dùng EP01 làm nguồn chính" → lấy 40 tập sinh sau làm chuẩn, gọi EP01 (gốc) là "bản nháp" (dòng 138) = **hợp thức hoá cái sai, đảo ngược nguyên tắc**. Mọi field-hoá/lock canon PHẢI neo tập sinh SỚM NHẤT làm anchor, ghi rõ tập nguồn.
> 3. **Nguyên nhân LỌT = KHÔNG KIỂM CHỨNG chéo.** Không có gate đối chiếu tập-sau-vs-tập-trước lúc sinh → EP02-50 tự trôi "ghế thứ ba" khác EP01 "ghế bảy" nhiều tháng không ai bắt; rồi bible codify theo số đông. **KHÓA CHẶT:** phải có cross-episode canon-consistency gate — với mỗi canon-fact recurring, trích giá trị TỪNG tập, nếu phân kỳ thì FLAG + chỉ tập sinh sớm nhất làm anchor (KHÔNG auto-tin số đông). Phân biệt *mâu thuẫn factual* (bắt) với *reveal tăng dần* hợp lệ (không bắt). **ĐÃ DỰNG (17/7):** `tools/cross_episode_canon_check.py` + `tests/test_cross_episode_canon_check.py` (5 test, mutation-proof: `test_divergence_anchors_on_earliest_not_majority` chứng anchor = min(ep) KHÔNG phải số đông). CHƯA wire vào ci_gate vì đang còn 1 phân kỳ THẬT chưa giải (EP01 ghế 7 / New York vs EP02-50 ghế 3 / Hà Nội — chờ Boss quyết sửa ~40 tập, xem TECH_DEBT DEBT-035 R216-reconcile); chạy tay để báo cáo.
> Vi phạm = process failure (R_SUPREME) — bug ở tập sau/bible, sửa xuôi theo tập trước + bổ sung gate chống tái diễn.

**Project:** SVHMP (Sài Gòn Hắc Mạ Phố / Hắc Dạ Ký narrative horror)
**Path:** `D:\DỰ ÁN AI\GIỌNG ĐỌC\DỰ ÁN TRUYỆN MA\SVHMP_Studio\`
**Last update:** 2026-07-16 (R215 điểm 6 — "đề xuất quy trình/nên quét định kỳ" không gate = cùng lớp rule-without-enforcer; rút từ DEBT-038 cp1252 promotion_guard)
