...
def main():
    ...
    # golden_lock: ca PASS-case (doc ep_01 that qua publish_score) va FAIL-case (ghi probe
    # vao ep_01 that) deu trong khoa cross-process -> khong giam nhau khi nhieu phien chay
    # dong thoi (FIX TRIET DE DEBT-005 — truoc day chi verify_post_fix duoc co lap).
    with golden_lock():
        ...
        try:
            ...
            # output/ep_01/episode_tts_ready.md — restored byte-for-byte from ready_orig
            # in the finally block below (process_ep() re-derivation khong byte-match golden).
