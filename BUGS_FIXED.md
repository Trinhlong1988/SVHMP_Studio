...
- **⚠ SUPERSEDED / REVERTED 17/7 — xem B_DEBT035_R216_reversal dưới đây.** Fix 16/7 này SAI HƯỚNG (sửa tập GỐC theo tập sau), vi phạm R216.

## B_DEBT035_R216_reversal (2026-07-17) — CMD_AUDIT (KIEM_DUYET), per Mr.Long R216 + "chốt Hướng A"

- **Bug quy trình (nghiêm trọng):** fix B_DEBT035_ep01_havy_death_canon (16/7) đã **sửa EP01 (tập sinh SỚM NHẤT, gốc) cho khớp EP11-50 (tập sinh SAU)** — reframe cái chết Hạ Vy New York→Hà Nội. Đây là ĐẢO NGƯỢC nguyên tắc: tập trước là gốc, tập sau mâu thuẫn = tập sau sai (R216, Mr.Long 17/7). Enforcer `canon_consistency_check.py` dựng 16/7 cũng khoá NHẦM hướng (coi New York là vi phạm) → cho false-confidence rằng sửa EP01 là đúng (4 tầng cùng sai 1 hướng: bible codify ngược → enforcer ngược → QA xanh → tin QA).
- **Bằng chứng hướng đúng (git, firsthand 17/7):** EP01 sinh `3306d91` **26/6**; EP11-50 auto-gen `c6a5d59` **27/6 17:06** (sau 1 ngày); bible/21 memory-arc `4cf9a78`/`148a80c` **27/6 tối**. → EP01 = ground truth. Boss xác nhận "chết ở nước ngoài là bình thường" → New York không phải world-error.
- **Fix (Hướng A — reconcile ngược về EP01):** (1) `output/ep_01` 3 file **REVERT** về v7 gốc (`git checkout cd6efcb`) — khôi phục "ghế số bảy" + "taxi New York / du học". (2) `bible/00` golden_reference **v8→v7** (`ep_01_v7_final_round3_lock`). (3) `tools/canon_consistency_check.py` + test (build ngược) **GỠ** (`git rm`). (4) Dựng enforcer đúng hướng `tools/cross_episode_canon_check.py` + 5 test (anchor = tập sớm nhất, KHÔNG tin số đông; mutation-proof `test_divergence_anchors_on_earliest_not_majority`). (5) Reframe note bible/03 (dòng 133-142) theo R216. (6) CLAUDE.md R216 hạ khung "làm rõ R65/R207 + vá 2 lỗ hổng enforcer" (KHÔNG phải luật mới — R65/R207 đã có).
- **Khóa chặt (R215 / R_SUPREME test_process_failure):** nguyên tắc gốc "cấm bịa xuyên tập" ĐÃ có (R65 bible/00:724 + R207 story_consistency_validator LOCKED_FIELDS gồm death_date/death_type/hometown). Lỗ hổng THẬT: (a) validator lấy baseline = bible/registry (không có tie-break "tập sớm nhất"); (b) chỉ đọc structured field, không đọc prose. `cross_episode_canon_check.py` vá (b) cho prose; R216 làm rõ (a).
- **CHƯA đóng (RECONCILE PENDING, khối lượng lớn):** bible/03 core_wound + bible/21 M-arc + roster + 49 tập EP02-50 vẫn mang version SAI (ghế-3/Hà Nội) → **regen bây giờ VẪN gen sai**. Đang scope bằng 3 agent → proposal `.md` + Boss duyệt trước khi phá bible/21 LOCK + regen. Xem `governance/TECH_DEBT.md` DEBT-035 Hướng A.
- **→ HOÀN TẤT trong cùng phiên (xem B_R216_SAGA dưới).**

## B_R216_SAGA (2026-07-17 END) — CMD_AUDIT, per Mr.Long "đập đi xây lại từ EP01" + "mày sáng tác cái gì đúng thì làm"

Nối tiếp B_DEBT035_R216_reversal → giải TRỌN VẸN trong phiên. 14 commit `fc196b5→f072f22`, mỗi cái CMD_AUDIT verify đối kháng (không tin agent self-report) + mutation-proof + CI-pass + verify `ls-remote`==local.

- **Truy gốc rễ (7-agent audit độc lập):** root cause gần MỌI bug = **CLAIM-ENFORCER GAP** (>54%: rule khẳng định "PHẢI/CẤM/đã-enforce" mà không có gate máy thật). Synthesis `governance/AUDIT_ROOT_CAUSE_SYNTHESIS_17_07.md`.
- **6 enforcer siết hiến pháp (mỗi cái mutation-proof, CMD_AUDIT tự mutation A/B/C):** #2 `gate_mutation_registry_check` (mọi ci_gate CHECK phải có mutation-test) · #1/#4 `claim_enforcer_registry_check` (13 claim TỐI THƯỢNG/DNA, bắt mask-as-active) · #3 `canon_anchor_check` (field-hoá canon phải khai anchor_ep+anchor_commit git-backed) · #5 relabel 5 enforcer mask-as-active (R61/68/69/71/72→DEBT-041) + **cp1252 backlog→0** (allowlist rỗng, đóng DEBT-038) + advisory (DEBT-042) · #6 khuôn pilot + `KHUON_ENGINE_DESIGN.md`.
- **Canon KHOÁ TRỌN VẸN về EP01:** death Hà Nội→New York (bible/03 core_wound, mọi câu trace dòng EP01) · seat ghế-3→ghế-7 (bible/03+21+canon_registry+bible/00+engine `auto_gen_ep` đọc canon) · **M11-M15 memory-arc REDESIGN** (Claude sáng tác bám EP01: KP KHÔNG chở/chết-cùng đêm tai nạn — "đi theo Hạ Vy MUỘN 8 năm"; Hạ Vy chết taxi New York một mình).
- **Engine (bằng chứng git):** máy viết chuẩn = **Claude/LLM** qua `prompts/generator.md`; `auto_gen_ep` = legacy template khô, KHÔNG dùng.
- **Phase-2 regen BẮT ĐẦU — pilot EP02 (`f072f22`):** Claude (writer-agent) viết lại về canon; KP=ghế7/Hạ Diệu=ghế3, bỏ bịa "mẹ KP chết", hồi ức M1 mơ hồ, sửa văn lỗi rename + bug giới tính, sync cả 2 file (episode+tts_ready, bài học DEBT-001). Gate: cross_episode '7'=[EP01,EP02], post_render 11 PASS, R86=0.
- **CÒN:** 47 tập (EP03-50 + archive) = marathon regen theo lô. Checklist per-tập: memory `project_svhmp_regen_readiness`.
- **Bài học codify:** R216 (tập sớm nhất=gốc) bible/00 + gate; verify tactics (3 bẫy) memory; sáng tác=Claude / code=agent memory.
